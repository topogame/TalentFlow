...
> **Important:** Phases 1A → 5 must be completed in order. Phase 6 is iterative (repeat for each feature). Phases 7–10 may overlap but should be approached in order. Never skip a phase — if a phase seems unnecessary, still do a quick pass and document why it was minimal.
...
**Error Scenarios:** Invalid email, weak password, verification email not received (resend option), expired verification link.
...
| Event | Channel | Recipient | Template | Frequency Limit |
|-------|---------|-----------|----------|-----------------|
| Welcome | Email | New user | Welcome + getting started | Once |
| Password Reset | Email | Requesting user | Reset link (expires in 1h) | Max 3/hour |
| New Message | In-app + Push | Recipient | Notification badge + push | Per message |
| ... | ... | ... | ... | ... |
...
- **Dependency risks:** Third-party API reliability, vendor lock-in, pricing changes
...
- **Financial risks:** Operational cost uncertainty, free tier limits, exchange rate exposure
...
| Service | Provider | Purpose | Cost | Fallback Plan |
|---------|----------|---------|------|---------------|
| Payment | ... | ... | ... | [alternative if provider fails/changes] |
| Email | ... | ... | ... | [alternative] |
...
## 8. Risk Assessment
| Risk | Likelihood | Impact | Mitigation | Trigger Point |
|------|-----------|--------|------------|---------------|
| [Technical risk] | H/M/L | H/M/L | [Strategy] | [When to act] |
| [Dependency risk] | ... | ... | ... | ... |
| [Timeline risk] | ... | ... | ... | ... |
| [Security risk] | ... | ... | ... | ... |
| [Financial risk] | ... | ... | ... | ... |
...
- **Present at least 2 viable options** with a comparison table:
  ...
  - Cost implications
  - **Migration path:** Can we start with option A and migrate to option B later if needed?
...
- **For each external dependency:** Document what happens if it becomes unavailable (fallback plan).
...
| Layer | Technology | Version | Justification | Fallback |
|-------|-----------|---------|---------------|----------|
...
## 9. External Dependency Risk Map
| Dependency | Purpose | Risk if Unavailable | Fallback Plan |
|-----------|---------|---------------------|---------------|
| [API/Service] | ... | [Impact on app] | [Alternative or graceful degradation] |
```
</template>
...
- **Every migration must have a corresponding down/rollback migration.** No exceptions.
...
### 3.7 — Backup & Recovery Strategy
- Define backup frequency (daily minimum for production).
- Define point-in-time recovery window.
- Document restore procedure.
- **Test the restore procedure at least once before launch.**
...
- Destructive migrations (drop column/table) require explicit approval
...
- **Public endpoints (no auth):** Strict limits (e.g., 10 req/min)
- **Authenticated endpoints:** Moderate limits (e.g., 100 req/min)
- **Sensitive endpoints (auth, payment):** Tight limits (e.g., 5 req/min)
- **Webhook endpoints:** Based on expected volume from provider
- Document how rate limit responses are formatted (429 + Retry-After header).
...
## Rate Limiting
| Endpoint Category | Limit | Response on Exceed |
|------------------|-------|-------------------|
| Public | 10 req/min | 429 + Retry-After |
| Authenticated | 100 req/min | 429 + Retry-After |
| Auth (login/register) | 5 req/min | 429 + Retry-After |
| Payment | 3 req/min | 429 + Retry-After |
...
- Race protection: UNIQUE constraint on email column prevents duplicate accounts
...
- **For non-technical users:** Always provide the complete copy-paste command sequence:
  ```
  git add .
...
- [ ] All inputs validated and sanitized (both client and server side)
...
- [ ] External service calls have timeouts and error handling
...
- Gaps: [any known untested scenarios and why]
```

**6.3.2 — UI Testing Guidance:**
After backend + tests are complete for a feature, ask the user to perform manual UI testing. Provide step-by-step instructions in plain Turkish:
```
🖥️ Lütfen tarayıcınızda şunları test edin:

1. [Adım 1 — ör. /login sayfasına gidin]
   Beklenen: [ne görmeniz gerekiyor]
2. [Adım 2 — ör. Geçersiz e-posta girin ve gönderin]
   Beklenen: [hata mesajı]
3. [Adım 3 — ör. Doğru bilgileri girin ve gönderin]
   Beklenen: [dashboard'a yönlendirme]
...

❌ Herhangi bir şey beklendiği gibi çalışmazsa, ne olduğunu bana söyleyin, düzeltirim.
✅ Tüm adımlar geçerse, onaylayın ve bir sonraki feature'a geçelim.
```
...
9. **Present bug fix summary** to the user (in plain Turkish):
   ```
   🐛 Bug Fix Summary:
   - Root cause: [what caused the bug — plain language]
   - Fix: [what was changed — plain language]
...
- Scan entire git history for accidentally committed secrets:
  ...
  ```
- If secrets were ever committed, rotate them immediately — even if the commit was later removed.
...
Generate a list for the user (in plain Turkish): happy paths, error scenarios, cross-feature interactions, load testing.
...
## 5. Manual Test Scenarios (for user — in Turkish)
| # | Senaryo | Adımlar | Beklenen Sonuç | Durum |
|---|---------|---------|----------------|-------|
| 1 | Tam kullanıcı yolculuğu | [adımlar] | [beklenen] | ☐ |
| 2 | Hata: geçersiz kayıt | [adımlar] | [beklenen] | ☐ |
| 3 | Çapraz özellik: [senaryo] | [adımlar] | [beklenen] | ☐ |
| ... | ... | ... | ... | ☐ |
...
## 8. Post-Deploy Smoke Test (for Murat)
After every deployment, test these 5 things:
- [ ] Ana sayfa açılıyor mu? (Homepage loads)
- [ ] Giriş yapılabiliyor mu? (Login works)
- [ ] Ana özellik çalışıyor mu? (Core feature works)
- [ ] Ödeme akışı çalışıyor mu? (Payment flow works — if applicable)
- [ ] Hata takip sistemi çalışıyor mu? (Error tracking receives test event)
```
</template>
...
- [ ] Compliance requirements met (GDPR/KVKK if applicable)
...
- **Feature audit:** For post-launch feature additions, use `Feature_Audit_Prompt.md` instead of the full pre-release audit. It focuses only on changed files and their blast radius.
- **Business strategy:** If the user needs a monetization/growth plan, recommend `Operational_Cost_Revenue_Analysis_Prompt.md` — AI analyzes the product it built and generates a comprehensive operational cost and revenue analysis with pricing, unit economics, and financial projections.
...
| `Pre_Release_Audit_Prompt.md` | Before launch | Rarely | Full project audit for initial release |
| `Feature_Audit_Prompt.md` | Post-launch | Rarely | Focused audit for new features/changes |
| `Operational_Cost_Revenue_Analysis_Prompt.md` | Pre/at launch | Quarterly | Operational costs, revenue, unit economics, projections |
| `AppStore_Review_Readiness_Prompt.md` | Before app store submission | Per submission | Apple/Google review compliance |
