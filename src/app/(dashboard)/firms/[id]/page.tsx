...
export default function FirmDetailPage() {
  ...
  const STATUS_LABELS: Record<string, string> = {
    open: tp("statusOpen"),
    on_hold: tp("statusOnHold"),
    closed: tp("statusClosed"),
  };

  const PRIORITY_LABELS: Record<string, string> = {
    low: tp("priorityLow"),
    normal: tp("priorityNormal"),
          ...
          <div className="mt-1 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                {[firm.sector, firm.city].filter(Boolean).join(" · ") || t("detail.noInfo")}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mt-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-900">{t("detail.processes", { count: processes.length })}</h3>
          </div>
        </div>
        <div className="mb-4">
          <input
            ...
            placeholder={t("detail.searchCandidate")}
        ...
        {filteredProcesses.length === 0 ? (
          <div className="py-6 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              {candidateSearch.trim() ? t("detail.noProcessMatch") : t("detail.noProcess")}
            </p>
          </div>
        ) : (
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-900">{t("detail.firmInfo")}</h3>
          </div>
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              <dt className="text-slate-500">{t("detail.companySize")}</dt>
            ...
            <div className="flex justify-between">
              <dt className="text-slate-500">{t("detail.website")}</dt>
              <dd>
                {firm.website ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-slate-900">{t("detail.positions", { count: firm.positions.length })}</h3>
            </div>
            <Link
          ...
          {firm.positions.length === 0 ? (
            <div className="py-6 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">{t("detail.noPositions")}</p>
            </div>
          ) : (
            <div className="space-y-2">
